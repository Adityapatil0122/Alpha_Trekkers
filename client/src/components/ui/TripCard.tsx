...
export default function TripCard({
  ...
  return (
    <motion.article
      ...
      className={`group h-full overflow-hidden border border-slate-200/70 bg-white shadow-[0_18px_50px_rgba(15,23,42,0.08)] ${
        isCompact ? 'rounded-[1.5rem]' : 'rounded-[1.35rem]'
      }`}
    >
      <Link to={`/trips/${trip.slug}`} className="flex h-full flex-col">
        <div className={`relative overflow-hidden ${isCompact ? 'h-56' : 'h-64'}`}>
          <img
        ...
        <div className={`relative flex flex-1 flex-col bg-white ${isCompact ? 'px-6 pb-5 pt-4' : 'rounded-t-[1.35rem] px-7 pb-7 pt-5'}`}>
          ...
          <h3 className={`mt-2.5 line-clamp-2 font-semibold leading-tight tracking-[-0.03em] text-slate-950 ${isCompact ? 'min-h-[3.9rem] text-[1.65rem]' : 'text-[2rem]'}`}>
            {trip.title}
          </h3>
